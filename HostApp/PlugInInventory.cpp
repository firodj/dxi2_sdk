...
HRESULT CPlugInInventory::EnumPlugIns()
{
	...
	for ( ULONG ixMoniker = 0; ixMoniker < cMoniker; ixMoniker++ )
	{
		...
		HKEY hKey = NULL;
		if (ERROR_SUCCESS == RegOpenKey( HKEY_CLASSES_ROOT, szKeyBuf, &hKey ))
		{
			if (hKey)
				RegCloseKey( hKey );
			bIsDXi = TRUE;
	...
	// Free moniker array
	for ( ixMoniker = 0; ixMoniker < cMoniker; ixMoniker++ )
...
HRESULT CPlugInInventory::GetInfo( ULONG ix, CLSID* pClsid, char* pszName, int cbName ) const
{
	if (ix >= m_cRegFilter)
		return E_INVALIDARG;
	if (NULL == pszName || NULL == pClsid)
		return E_POINTER;

	*pClsid = m_aRegFilter[ ix ].Clsid;
	WideCharToMultiByte( CP_ACP, 0, m_aRegFilter[ ix ].Name, -1, pszName, cbName, NULL, NULL );
	return S_OK;
}
