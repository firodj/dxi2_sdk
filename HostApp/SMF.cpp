	...
	if (rFile.Read( szChunkHeader, 4 ) == 4)				// Chunk name
	{
		...
		*pdwLen = readSMFDword( rFile, phr );		// Chunk length
		...
		if (S_OK == *phr)
		{
			...
			if (lstrcmp( szChunkHeader, "MThd" ) == 0)
				return MThd;
				//////

			if (lstrcmp( szChunkHeader, "MTrk" ) == 0)
				return MTrk;
				//////
...
public:
	...
	BOOL SetText( const char* szString )
	{
		...
		m_nLength = lstrlen( szString );
		delete [] m_pszString;
		m_pszString = new char[ m_nLength + 1 ];
		if (m_pszString)
		{
			lstrcpy( m_pszString, szString );
	...
	const char* GetStringPtr()	{  return m_pszString;  }
	char* 		GetHotStringPtr()	{  return m_pszString;  }
	int 			GetLength()		{  return m_nLength;  }
	BOOL 			IsEmpty()		{  return (m_nLength > 0)? FALSE : TRUE;  }
			...
			while (m_pszString[ ix ])
			{
				if ((m_pszString[ ix ] == CR) || (m_pszString[ ix ] == LF))
				{
						 ...
						 ((m_pszString[ ix ] != m_pszString[ ix+1 ])))
					{
						...
						lstrcpy( pszNewString, m_pszString );
						lstrcpy( pszNewString + ix + 1, m_pszString + ix );
	...
	BOOL Append( const char* szAppend )
	{
		...
		char* pszNewString = new char[ m_nLength + lstrlen( szAppend ) + 1 ];
		if (!pszNewString)
			return FALSE;
		...
		if (m_pszString)
		{
			lstrcpy( pszNewString, m_pszString );
			delete [] m_pszString;
			lstrcat( pszNewString, szAppend );
		}
		else
			lstrcpy( pszNewString, szAppend );
		m_pszString = pszNewString;
		m_nLength += lstrlen( szAppend );
		m_pszString[ m_nLength ] = '\0';
	...
	BOOL Prepend( const char* szPrepend )
	{
		...
		char* pszNewString = new char[ m_nLength + lstrlen( szPrepend ) + 1 ];
		if (!pszNewString)
			return FALSE;

		lstrcpy( pszNewString, szPrepend );
		if (m_pszString)
		{
			lstrcat( pszNewString, m_pszString );
			delete [] m_pszString;
		}
		m_pszString = pszNewString;
		m_nLength += lstrlen( szPrepend );
		m_pszString[ m_nLength ] = '\0';
...
inline BOOL isPatch100BankForPatch( const CMfxEvent& evtPatch1, const CMfxEvent& evtPatch2 )
{
	...
	return
		evtPatch1.GetType() == CMfxEvent::Patch &&			// Is patch?
							 ...
							 CReadQueue& readQueue, HRESULT* phr )
{
	...
	enum	{ ECHAN_0=0, ECHAN_15=15, ECHAN_NONE, ECHAN_MULTI };
	int   ixBankPatch = -1;
					...
					// Lyrics after time 0 get loaded as normal lyric events...
					{
						...
						if (pTrk)
						{
							...
							if (S_OK == *phr)
							{
								...
								int iLen = lstrlen( spLyric.GetStringPtr() );
