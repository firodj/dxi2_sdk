...
CMfxHostSite::CMfxHostSite() : m_cRef(0), m_pSeq(NULL), m_nPulseRequests(0)
{
}
...
static LONGLONG GetSamples( MFX_TIME* pTime )
{
	if (TF_SAMPLES == pTime->timeFormat)
		return pTime->llSamples;
	else if (TF_SECONDS == pTime->timeFormat)
		return static_cast<LONGLONG>( pTime->dSeconds * SAMPLE_RATE );
	else
	{
		ASSERT(FALSE);
		return 0;
...
HRESULT CMfxHostSite::ConvertMfxTime( MFX_TIME* pTime, MFX_TIME_FORMAT newFormat )
{
	...
	if (bInMusical == bOutMusical)
	{
		...
		else
		{
			...
			LONGLONG llSamples = GetSamples( pTime );
			if (TF_SECONDS == newFormat)
				pTime->dSeconds = static_cast<double>( llSamples ) / SAMPLE_RATE;
			else
	...
	else
	{
		if (bInMusical)
		{
			...
			LONGLONG llUTicks = GetUTicks( GetSeq(), pTime );
			LONGLONG llSample = GetSeq()->m_tempoMap.UTick2Sample( llUTicks, SAMPLE_RATE );
			if (TF_SAMPLES == newFormat)
				pTime->llSamples = llSample;
			else
				pTime->dSeconds = static_cast<double>( llSample ) / SAMPLE_RATE;
		}
		else
		{
			// Convert absolute to musical
			LONGLONG llSample = GetSamples( pTime );
			LONGLONG llUTicks = GetSeq()->m_tempoMap.Sample2UTick( llSample, SAMPLE_RATE );
			if (TF_UTICKS == newFormat)
				pTime->llUTicks = llUTicks;
			else
			{
