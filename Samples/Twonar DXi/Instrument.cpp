...
HRESULT CInstrument::GetInstrumentName( char* pszName, int cbName )
{
	if (NULL == pszName)
		return E_POINTER;
	if (cbName < m_strName.GetLength() + 1)
		return E_POINTER;

	strcpy( pszName, m_strName );
	return S_OK;
}
...
HRESULT CInstrument::createPatchNames( int nBank )
{
	...
	CString		str = m_strName + " Patches";
	CStringMap*	pStringMap = NULL;
	HRESULT hr = createStringMap( &pStringMap, str, 128 );
	if (FAILED( hr ))
		return hr;
	m_mapBank2PatchNames[ nBank ] = pStringMap;

	if (FAILED( hr = pStringMap->SetAt( 0, "Sine" ) ))
		return hr;
	if (FAILED( hr = pStringMap->SetAt( 1, "Square" ) ))
		return hr;
	if (FAILED( hr = pStringMap->SetAt( 2, "Saw-Tooth" ) ))
		return hr;
...
HRESULT CInstrument::createControllerNames()
{
	...
	CString str = m_strName + " Controllers";
	HRESULT hr = createStringMap( &m_pMapController, str, 128 );
	if (FAILED( hr ))
		return hr;

	if (FAILED( hr = m_pMapController->SetAt( 7, "Volume" ) ))
		return hr;
	if (FAILED( hr = m_pMapController->SetAt( 10, "Pan" ) ))
		return hr;
...
HRESULT CInstrument::createRpnNames()
{
	...
	CString str = m_strName + " RPNs";
	HRESULT hr = createStringMap( &m_pMapRpn, str, 16383 );
	if (FAILED( hr ))
		return hr;
...
HRESULT CInstrument::createNrpnNames()
{
	...
	CString str = m_strName + " NRPNs";
	HRESULT hr = createStringMap( &m_pMapNrpn, str, 16383 );
	if (FAILED( hr ))
		return hr;
