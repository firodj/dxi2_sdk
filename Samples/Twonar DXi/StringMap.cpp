...
HRESULT CStringMap::GetTitle( char* pszString, int cbString )
{
	...
	if (cbString > 0)
	{
		strncpy( pszString, m_strTitle, cbString );
...
{
	if (NULL == pszName && cbName > 0 && !AfxIsValidAddress( pszName, cbName, TRUE ))
		return E_POINTER;
	if (ix < 0 || ix >= m_nMaxNames)
		return E_INVALIDARG;

	map<int,CString>::iterator it = m_map.find( ix );
	if (it != m_map.end())
	{
		strncpy( pszName, (*it).second, cbName );
		pszName[ cbName - 1 ] = 0;
	}
	else
	{
...
HRESULT CStringMap::GetStartPosition( MFX_POSITION* pPos )
{
	...
	ASSERT( sizeof(MFX_POSITION) == sizeof( map<int,CString>::iterator ) );
	if ( sizeof(MFX_POSITION) != sizeof( map<int,CString>::iterator ) )
		return E_FAIL;
...
HRESULT CStringMap::GetNextAssoc( MFX_POSITION* pPos, int* pnKey, char* pszString, int cbString )
{
	...
	if (NULL == pszString && cbString > 0 && !AfxIsValidAddress( pszString, cbString, TRUE ))
		return E_POINTER;
	if (NULL == pPos)
		return E_POINTER;
	...
	map<int,CString>::iterator it = *reinterpret_cast<map<int,CString>::iterator*>( pPos );
	...
	{
		strncpy( pszString, (*it).second, cbString );
		pszString[ cbString - 1 ] = 0;
	}
...
HRESULT CStringMap::Lookup( int nKey, char* pszString, int cbString )
{
	...
	map<int,CString>::iterator it = m_map.find( nKey );
	if (it != m_map.end())
	{
		if (NULL != pszString && cbString > 0)
		{
