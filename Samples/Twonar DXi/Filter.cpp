...
const GUID CLSID_Filter =
	{ 0x68b257ea, 0xe0cd, 0x4f28, { 0x9c, 0xe8, 0xe2, 0x6f, 0x7a, 0x2c, 0x71, 0xa7 } };

const GUID CLSID_FilterPropPage =
	{ 0x63b6c36e, 0xadd0, 0x46a9, { 0xaf, 0xaf, 0x77, 0x12, 0x16, 0xef, 0x9a, 0xd0 } };
...
static AMOVIESETUP_FILTER sudFilter =
{
    ...
    L"Twonar Plug-In",         // Filter's name
...
CFactoryTemplate g_Templates[ 2 ] =
{
    ...
    , &sudFilter }
	 ...
	 , CTwonarPropPage::CreateInstance
	 , NULL
	 , NULL }
};
...
{
	if (riid == IID_ITwonar) 
		return GetInterface((ITwonar*)this, ppv);
...
static HRESULT maybeLoadTypeInfo()
{
	...
	{
		ITypeLib* ptl;
		if (SUCCEEDED( LoadRegTypeLib( LIBID_TwonarTypeLibrary, 1, 0, MAKELCID(0, SORT_DEFAULT), &ptl ) ))
		{
			ptl->GetTypeInfoOfGuid( IID_ITwonar, &m_pTypeInfo );
			ptl->Release();
		}
		else
			return E_NOTIMPL;
...
HRESULT CFilter::GetTypeInfoCount( UINT* pctInfo )
{
	*pctInfo = SUCCEEDED( maybeLoadTypeInfo() ) ? 1 : 0;
	return NOERROR;
}

HRESULT CFilter::GetTypeInfo( UINT itInfo, LCID lcid, ITypeInfo** ppITypeInfo )
{
	if (itInfo != 0)
		return E_INVALIDARG;
...
HRESULT CFilter::GetIDsOfNames( REFIID riid, OLECHAR** rgszNames, UINT cNames,
...
HRESULT CFilter::Invoke( DISPID dispID, REFIID riid, LCID lcid,
								 ...
								 VARIANT* pVarResult, EXCEPINFO* pExcepInfo, UINT* puArgErr )
{
	...
	if (FAILED( maybeLoadTypeInfo() ))
		return E_NOTIMPL;
	return m_pTypeInfo->Invoke( (ITwonar*)this, dispID, wFlags,
...
HRESULT CFilterInputPin::Receive( IMediaSample* pms )
{
	...
	AudioBuffer bufIn;
	bufIn.cSamp = pms->GetActualDataLength() / m_pFilter->m_wfxIn.nBlockAlign;
	bufIn.pms = pms;
	...
	HRESULT hrProcess = m_pFilter->Process( m_pFilter->m_llSamplePosition, &bufIn, abufOut, cBufOut,
	...
	// Update our position
	m_pFilter->m_llSamplePosition += bufIn.cSamp;
