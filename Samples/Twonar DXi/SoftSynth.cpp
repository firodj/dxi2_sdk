...
CSoftSynth::CSoftSynth( CDXi* pDXi, ITwonar* pTwonar, HRESULT* phr ) :
	m_pDXi( pDXi ),
	m_pTwonar( pTwonar ),
	m_pTempoMap(NULL),
	m_pMeterMap(NULL),
...
{
	ASSERT( NULL != m_pDXi );
	m_pDefaultInstrument = new CInstrument("Twonar");
...
void CSoftSynth::releaseInterfaces()
{
	...
	if (m_pMeterMap)
		m_pMeterMap->Release(), m_pMeterMap = NULL;
	if (m_pTimeConverter)
...
STDMETHODIMP CSoftSynth::Connect( IUnknown* pContext )
{
	...
	// We need a meter map to do the metronome
	pContext->QueryInterface( IID_IMfxMeterMap, (void**)&m_pMeterMap );
...
LONG CSoftSynth::metroGetNextTick()
{
	ASSERT( TF_MBT == m_tNextMetro.timeFormat );
	ASSERT( 0 == m_tNextMetro.mbt.nTick );

	// How many beats are in effect at the current time?
	MFX_TIME tNow = m_tNextMetro;
	m_pTimeConverter->ConvertMfxTime( &tNow, TF_TICKS );
	return tNow.lTicks;
}

//////////////////////////////////////////////////////////////////////

void CSoftSynth::metroNextBeat()
{
	if (NULL == m_pMeterMap)
		return; // no metronome is possible

	ASSERT( TF_MBT == m_tNextMetro.timeFormat );
	ASSERT( 0 == m_tNextMetro.mbt.nTick );

	// How many beats are in effect at the current time?
	LONG const lNow = metroGetNextTick();
	int ixMeter = m_pMeterMap->GetMeterIndexForTime( lNow );
	int nMeterTop = 1;
	if (-1 != ixMeter)
	{
		int nMeas, nBottom;
		m_pMeterMap->GetMeterAt( ixMeter, &nMeas, &nMeterTop, (IMfxMeterMap::EBeatValue*)&nBottom );
	}

	// Advance to next beat
	m_tNextMetro.mbt.nBeat++;
	if (m_tNextMetro.mbt.nBeat > nMeterTop)
	{
		m_tNextMetro.mbt.nBeat = 1;
		m_tNextMetro.mbt.nMeas++;
	}
}
...
STDMETHODIMP CSoftSynth::OnStart( LONG lTime )
{
	...
	// Convert tick start time to MBT
	m_tNextMetro.timeFormat = TF_TICKS;
	m_tNextMetro.lTicks = lTime;
	m_pTimeConverter->ConvertMfxTime( &m_tNextMetro, TF_MBT );

	// Advance to next beat if necessary
	if (m_tNextMetro.mbt.nTick != 0)
	{
		m_tNextMetro.mbt.nTick = 0;
		metroNextBeat();
	}
...
STDMETHODIMP CSoftSynth::OnLoop( LONG lTimeRestart, LONG lTimeStop )
{
	...
	// Convert tick start time to MBT
	m_tNextMetro.timeFormat = TF_TICKS;
	m_tNextMetro.lTicks = lTimeRestart;
	m_pTimeConverter->ConvertMfxTime( &m_tNextMetro, TF_MBT );
...
STDMETHODIMP CSoftSynth::OnEvents( LONG lTimeFrom, LONG lTimeThru, MFX_CHANNEL mfxChannel, IMfxEventQueue* pqIn )
{
	...
	// Generate metronome beeps
	if (NULL != m_pMeterMap && NULL != m_pTwonar)
	{
		// Get notes and velocities for metronome
		int nNoteMeasTop = 0, nVelMeasTop = 0, nNoteBeat = 0, nVelBeat = 0;
		m_pTwonar->get_MetroInfo( &nNoteMeasTop, &nVelMeasTop, &nNoteBeat, &nVelBeat );

		// Generate one metronome beep on every beat
		LONG lMetroTick = metroGetNextTick();
		while (lTimeFrom <= lMetroTick && lMetroTick < lTimeThru)
		{
			// Determine whether metronome is active or not
			BOOL bEnableMetro = FALSE;
			m_pTwonar->get_EnableMetro( &bEnableMetro );
			if (bEnableMetro)
			{
				BOOL const bMeasTop = (1 == m_tNextMetro.mbt.nBeat);
				DXiEvent de;
				
				// Convert the event start time from ticks to samples, relative to
				// the start of the project.
				LONGLONG llSampTime = TicksToSamples( lMetroTick );
				
				// Set MFX properties
				de.me.m_byChan = 0;
				de.me.m_lTime = lMetroTick;
				de.me.m_byPort = 0;
				de.me.m_byVelOff = 0;
				de.me.m_eType = MfxEvent::Note;
				de.me.m_byKey = (BYTE)( (bMeasTop) ? nNoteMeasTop : nNoteBeat );
				de.me.m_byVel = (BYTE)( (bMeasTop) ? nVelMeasTop : nVelBeat );
				de.me.m_dwDuration = 960 / 32;
			...
			// Do the next metronome tick
			metroNextBeat();
			lMetroTick = metroGetNextTick();
		}
	}
