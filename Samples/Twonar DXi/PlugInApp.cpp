...
static const char* s_pszReg = "MfxSoftSynths\\";

extern "C" const GUID LIBID_TwonarTypeLibrary;
...
STDAPI DllRegisterServer()
{
	...
	GetModuleFileName( theApp.m_hInstance, sz, sizeof sz );
	if (0 == MultiByteToWideChar( CP_ACP, 0, sz, _MAX_PATH, wsz, _MAX_PATH ))
		goto DONE;
		
	// Register our type library if we've got one
	if (S_OK == LoadTypeLib( wsz, &pTypeLib ))
	{
		RegisterTypeLib( pTypeLib, wsz, NULL );
		pTypeLib->Release();
	}
...
{	
	...
	// Unregister our type library if we've got one
	UnRegisterTypeLib( LIBID_TwonarTypeLibrary, 1, 0, MAKELCID(0, SORT_DEFAULT), SYS_WIN32 );
