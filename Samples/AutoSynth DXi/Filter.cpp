...
const GUID CLSID_Filter =
	{ 0x039ef282, 0xc745, 0x4f2e, { 0x8f, 0x82, 0xe2, 0x8a, 0x1f, 0x39, 0x08, 0xb0 } };

const GUID CLSID_FilterPropPage =
	{ 0x8206719d, 0xa9d2, 0x4f2d, { 0x8a, 0xf1, 0x46, 0x48, 0xdd, 0xaf, 0x4e, 0x51 } };
...
static AMOVIESETUP_FILTER sudFilter =
{
    ...
    L"AutoSynth Plug-In",         // Filter's name
...
CFactoryTemplate g_Templates[ 2 ] =
{
    ...
    , &sudFilter }
	 ,
	 { L"AutoSynth Properties"
	 , &CLSID_FilterPropPage
	 , CAutoSynthPropPage::CreateInstance
	 , NULL
	 , NULL }
};
...
HRESULT CFilterInputPin::Receive( IMediaSample* pms )
{
	...
	for (std::vector<LONGLONG>::iterator it = samplePos.begin(); it != samplePos.end(); it++)
	{
		...
		long const cSampIter = min( llCurrPos - llPrevPos, cSampTotal - cSampDone );
		if (0 == cSampIter)
