														...
														std::vector<LONGLONG>* pTimes )
{
	...
	REFERENCE_TIME const	rtStart = llSampStart / dSamplesPerRefTime + 0.5;
	...
	uPoints += ((llSampEnd - llSampStart) / llInterval) + 1;
...
{
	double const			dSamplesPerRefTime = static_cast<double>( GetSampleRate() ) / UNITS;
	REFERENCE_TIME const	rt = llSamp / dSamplesPerRefTime + 0.5;
